...
The config is read from the fixed path `/etc/swhkdp/config.kdl`.
Release builds have **no** `-c`/`--config` flag.
For debug builds keept `-c <path>` for testing purposes.

Because the daemon parses the config as root and keys can be remapped from it,
before parsing it requires:

- the **file** to be owned by root and writable only by root
- every **parent directory** to likewise be root-owned and root-write-only, so
  no non-root users can't replace or link file to config dir
